...
Two further skills serve teaching workspaces rather than the planning loop: `distill-dialogue` captures a conversation's material as a handoff package, and `integrate-lesson` lands that package in the course documents from a fresh session.
...
| `distill-dialogue` | **New** | Extracts the valuable material from a conversation into a one-off handoff package (Markdown) for a separate session to integrate: fidelity rules, provenance and a must-land list. | Saving discussion material to integrate into course documents later | **Manual** |
| `integrate-lesson` | **New** | Integrates a handoff package into course HTML from a fresh session: reads the workspace rules and the document as a whole, places, merges or corrects material by its logic, verifies, then reports where every must-land item went. | Landing a handoff package in the course documents | **Manual** |
