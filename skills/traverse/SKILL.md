...
**Layer integrity check.** For each node, compare its claimed layer against its body content. A node in the scan layer whose body describes engine-level loops → **violation**. Report it — no question needed, the document evidence is clear.
