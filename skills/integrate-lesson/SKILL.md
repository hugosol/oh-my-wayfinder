...
description: Integrate a landing draft (Markdown) into course documents from a separate session.
...
- Take only the clauses this integration needs: document organization, material placement (main line vs on-demand vs companion pages, by lifetime), definitions and terminology, progressive-disclosure sections, links and anchors, verification.
...
- Decide each section's placement yourself: add / merge into an existing passage / already covered / conflicts with existing text / route to a companion page / undecided.
...
- Placement follows the document's logic and flow — never the draft's order, never the spot where the question happened to be asked.

Done when: every section has a placement decision, or is marked for the user to decide.
...
Done when: every section has a reported placement, and the user has decided the draft's fate.
...
- Reporting "done" without the per-section placements.
